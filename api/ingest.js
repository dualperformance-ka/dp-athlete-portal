...
export default async function handler(req, res) {
  ...
  if (!targetUrl || !ALLOWED_TARGETS.has(targetUrl)) return send(res, 400, { ok: false, error: 'Invalid targetUrl' });
  if (!athleteCode(payload) && targetUrl !== '/api/notion') return send(res, 400, { ok: false, error: 'athleteCode is required' });
