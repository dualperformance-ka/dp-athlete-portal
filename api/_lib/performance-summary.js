/**
 * Weekly performance summary — server aggregation.
 *
 * WHAT THIS IS
 * One authenticated read behind `/api/portal-data` (action `performance-summary`)
 * that turns the athlete's own server-held rows into a factual weekly summary.
 * The browser sends a programme-week UUID and nothing else: no dates, no totals,
 * no athlete code. Everything the summary reports is derived here.
 *
 * WHY THE SPLIT
 * Everything above the `── DATABASE ──` divider is pure: same inputs, same
 * output, no I/O, no clock unless a date is handed in. That is the half the
 * tests exercise directly, and the half a later monthly report is expected to
 * reuse rather than reinvent (see docs/weekly-performance-summary.md).
 *
 * PRIVACY BOUNDARY (deliberate, do not loosen)
 * - Raw Strava rows never leave this module. Only aggregates — session count,
 *   distance, duration, and which source produced them — reach the response.
 *   The Strava API Agreement permits a user's data to be shown back to THAT
 *   USER only, so nothing here may ever be reused for a coach-facing surface.
 * - Check-in free text, daily-log notes, internal ids and raw sets are read for
 *   the maths and dropped. The response carries counts and numbers.
 *
 * NULL vs ZERO
 * `null` means unavailable or not measurable. `0` means a verified count of
 * zero. An optional source that fails must produce `null`, never `0` — a
 * silent zero reads to an athlete as "you did nothing this week".
 */
...
// ── DATABASE ─────────────────────────────────────────────────────────────────
//
// Everything below here reads Supabase. Mandatory reads (ownership, the week
// row, the planned sessions) fail the request. Optional reads degrade: the rest
// of the summary still renders, the affected metrics go null, and the source is
// named in dataQuality.missingSources.
...
// PostgREST names a missing column in its error, so an optional projection can
// be retried without it rather than losing the whole read. Same idea as
// upsertTolerant in api/ingest.js, applied to selects.
export async function selectTolerant(table, query, optionalColumns = [], selectRows = select) {
  let columns = String(query.select || '').split(',').filter(Boolean);
  for (let attempt = 0; attempt <= optionalColumns.length; attempt += 1) {
    try {
      return await selectRows(table, { ...query, select: columns.join(',') });
    } catch (error) {
      const message = String((error && error.message) || '');
      const missing = optionalColumns.find(
        (column) => columns.includes(column) && message.includes(column),
      );
      if (!missing) throw error;
      console.warn(`[performance-summary] ${table}.${missing} not present — retrying without it`);
      columns = columns.filter((column) => column !== missing);
    }
  }
  return selectRows(table, { ...query, select: columns.join(',') });
}
...
export async function performanceSummary(code, body = {}, deps = {}) {
  ...
  const programmeIds = (Array.isArray(programmes) ? programmes : []).map((row) => row.id).filter(Boolean);
  if (!programmeIds.length) throw requestError('Programme week not found', 404);

  const weeks = await selectRows('athlete_programme_weeks', {
    id: `eq.${programmeWeekId}`,
    programme_id: `in.(${programmeIds.join(',')})`,
    select: 'id,programme_id,week_number,week_label,start_date',
    limit: '1',
  });
  const weekRow = (Array.isArray(weeks) ? weeks : [])[0];
  if (!weekRow) throw requestError('Programme week not found', 404);
  ...
  const optional = (name, promise, fallback) => promise.then(
    (value) => value,
    (error) => {
      // The raw database message may name internals, so it is logged and never
      // returned. The client learns only which named source is missing.
      console.warn(`[performance-summary] ${name} read failed:`, error && error.message);
      missingSources.push(name);
      return fallback;
    },
  );

  const structuredIds = planned
    .filter((row) => row.prescription_mode === 'structured')
    .map((row) => row.id)
    .filter(Boolean);

  // The keys this week's completion can possibly match. Notion ids and UUIDs
  // are alphanumeric with dashes, so they need no quoting inside in.(); a key
  // carrying anything else is dropped rather than allowed to break the filter.
  ...
  const [
    ...
    // Scoped to THIS week's keys rather than reading the whole log. An
    // unbounded read caps at some limit and truncates arbitrarily once an
    // athlete has enough history, which would quietly under-report completion
    // — the worst possible failure for the headline number on the card.
    plannedKeys.length
    ...
    optional('training_session_logs', selectTolerant('training_session_logs', {
      athlete_code: `eq.${code}`,
      and: `(session_date.gte.${startDate},session_date.lte.${endDate})`,
      select: TRAINING_LOG_COLUMNS,
      order: 'session_date.asc',
      limit: '1000',
    }, TRAINING_LOG_OPTIONAL, selectRows), null),
    optional('strength_history', selectTolerant('training_session_logs', {
      athlete_code: `eq.${code}`,
      session_date: `lte.${endDate}`,
      select: 'session_name,session_category,session_date,exercise_name,programmed_exercise,raw_sets',
      order: 'session_date.asc',
      limit: '5000',
    }, ['exercise_name', 'programmed_exercise'], selectRows), null),
    optional('daily_body_logs', selectRows('daily_body_logs', {
      athlete_code: `eq.${code}`,
      and: `(log_date.gte.${startDate},log_date.lte.${endDate})`,
      select: 'log_date,weight,sleep,energy,stress,soreness,raw_payload',
      order: 'log_date.asc',
      limit: '31',
    }), null),
    optional('daily_body_logs_previous', selectRows('daily_body_logs', {
      athlete_code: `eq.${code}`,
      and: `(log_date.gte.${previousStart},log_date.lte.${previousEnd})`,
      select: 'log_date,sleep,energy,stress,soreness',
      order: 'log_date.asc',
      limit: '31',
    }), null),
    optional('weekly_checkins', selectRows('weekly_checkins', {
      athlete_code: `eq.${code}`,
      and: `(week_ending.gte.${startDate},week_ending.lte.${endDate})`,
      select: 'week_ending,submitted_at',
      order: 'submitted_at.desc',
      limit: '10',
    }), null),
    // Aggregate columns only. The `summary` jsonb — the raw Strava payload —
    // is deliberately not projected, so it cannot reach the response by
    // accident even if a later change starts echoing rows.
    // start_date_local is timestamptz. Comparing it to a bare date makes
    // Postgres cast using the SERVER timezone — UTC on Vercel — so an Adelaide
    // 06:00 Monday run (20:30 UTC on Sunday) falls outside a `gte.<startDate>`
    // filter and silently disappears from the week. The window is deliberately
    // a day wider at each end; aggregateActualEndurance() then enforces the
    // exact boundary on the row's own local date string, which is the only
    // representation that means what the athlete means by "Monday".
    optional('strava_activities', selectRows('strava_activities', {
      athlete_code: `eq.${code}`,
      and: `(start_date_local.gte.${addDaysISO(startDate, -1)},start_date_local.lt.${addDaysISO(endDate, 2)})`,
      select: 'sport_type,start_date_local,distance_m,moving_time_s,elapsed_time_s',
      order: 'start_date_local.asc',
      limit: '200',
    }), null),
    optional('workout_splits', selectRows('workout_splits', {
      archived: 'eq.false',
    ...
    structuredIds.length
      ? optional('session_exercises', selectRows('session_exercises', {
        planned_session_id: `in.(${structuredIds.join(',')})`,
        select: 'planned_session_id',
        limit: '2000',
      }), null)
      : Promise.resolve([]),
    structuredIds.length
      ? optional('run_steps', selectRows('run_steps', {
        planned_session_id: `in.(${structuredIds.join(',')})`,
        select: 'planned_session_id',
        limit: '2000',
      }), null)
  ...
  (Array.isArray(exerciseRows) ? exerciseRows : []).forEach((row) => {
    const entry = structured.get(row.planned_session_id);
    if (entry) entry.exercises += 1;
  });
  (Array.isArray(runStepRows) ? runStepRows : []).forEach((row) => {
    const entry = structured.get(row.planned_session_id);
    if (entry) entry.runSteps += 1;
  });
  ...
  const gymKeys = BASE_GYM_KEYS.concat(
    (Array.isArray(splitRows) ? splitRows : [])
      ...
      .filter((name) => name && !BASE_GYM_KEYS.includes(name)),
  ).sort((a, b) => b.length - a.length);

  const warnings = [];
  // Without session_logs there is no authoritative completion signal, so the
  // status fallback would silently under-report. Say so rather than imply the
  // athlete logged nothing.
  if (loggedRows === null) warnings.push('completion is based on planned-session status only');

  // PBs need the full history. Without it a "PB this week" would be measured
  // against an empty past and every set would look like a record.
  const personalBestsStatus = strengthHistoryRows === null ? 'not_calculated' : 'calculated';
  ...
  return {
    summary: buildSummary({
      ...
      plannedRows: planned,
      loggedKeys,
      trainingLogs: Array.isArray(trainingLogRows) ? trainingLogRows : [],
      ...
      stravaRows: Array.isArray(stravaRows) ? stravaRows : null,
      gymKeys,
      structured,
      todayISO,
