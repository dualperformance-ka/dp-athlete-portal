...
You are working in the dp-athlete-portal repo, a mobile-first PWA at portal.dualperformance.au. Vanilla JS, no framework. Read these before you touch anything: `docs/phase-4-prompt.md` (the design system rebuild brief, its non-negotiable repo rules govern this work), `public/styles.css` (the `:root` token block at the top), `public/desktop.css`, `public/index.html`, `public/js/01-core.js` through `10-boot.js`, and `scripts/check-portal.mjs`.

This work is the visual layer of Phase 4. Everything in `docs/phase-4-prompt.md` still applies, including the rule that the Playwright suite must be green in CI before the cascade is touched, because the `node --test` suite asserts source as text and will stay green through a refactor that visually destroys the app. If Playwright is not running, stop and say so.
...
- `check-portal.mjs` reads specific files for specific literal strings, including a literal CSS string in `styles.css` and markers that must exist in `js/08-training.js` or `styles.css`. Read the script before editing the cascade. It also asserts the home DOM layout, so any reordering happens in JS at runtime, not in the HTML.
- `api/` stays under 24 files, which supersedes the stale 12-file ceiling in `docs/phase-4-prompt.md`. Nothing in this work needs a server change anyway.
- Do not touch the reminder scheduler.
