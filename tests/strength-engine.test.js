...
test('a live load decrease is kept and resets remaining reps to the programmed floor', () => {
  const pres = prescribe({ exercise: 'Hip Abduction', sets: '4', workingSets: '4', repRange: '15-20' });
  const live = strengthLiveLoadDecision({
    prescription: pres,
    sets: normaliseStrengthSets([
      { _rowIndex: 0, weight: '68', reps: '18', done: true },
      { _rowIndex: 1, weight: '63', reps: '' },
    ], pres),
    referenceSets: rows(pres, [set(68, 18), set(68, 16), set(68, 15), set(68, 15)]),
  });
  assert.equal(live.decision, 'live_load_decrease');
  assert.equal(live.direction, 'easier');
  assert.equal(live.currentLoad, 63);
  assert.equal(live.targetLoad, 63, 'the engine does not undo the athlete’s manual reduction');
  assert.equal(live.repTarget, 15);
  assert.equal(live.startWorkingIndex, 1);
  assert.match(live.prompt, /Aim for at least 15 clean reps/);
});

test('the most recent same-workout load is the live comparison point', () => {
  const pres = prescribe({ exercise: 'Machine Press', repRange: '10-12' });
  const live = strengthLiveLoadDecision({
    prescription: pres,
    sets: normaliseStrengthSets([
      { _rowIndex: 0, weight: '55', reps: '10', done: true },
      { _rowIndex: 1, weight: '50', reps: '' },
    ], pres),
    referenceSets: rows(pres, [set(50, 12), set(50, 11), set(50, 10)]),
  });
  assert.equal(live.decision, 'live_load_decrease');
  assert.equal(live.previousLoad, 55, 'today’s immediately preceding working load beats stale history');
  assert.equal(live.currentLoad, 50);
  assert.equal(live.repTarget, 10);
});
