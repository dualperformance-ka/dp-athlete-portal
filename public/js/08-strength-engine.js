...
function _seIsEasier(pres,candidate,reference){
  return pres.assisted?candidate>reference+0.01:candidate<reference-0.01;
}
...
function strengthLiveLoadDecision(input){
  ...
  // A different load already used earlier today is the most immediate point of
  // comparison. Otherwise compare with the previous confirmed workout.
  ...
  var direction=_seIsHarder(pres,candidate,reference)?'harder':(_seIsEasier(pres,candidate,reference)?'easier':'same');
  ...
  for(var j=0;j<current.length;j++){
    if(!current[j].hasLoad)continue;
    var changedFromReference=direction==='harder'
      ?_seIsHarder(pres,current[j].load,reference)
      :_seIsEasier(pres,current[j].load,reference);
    if(changedFromReference){
  ...
  if(pain){
    return {
      ...
      direction:direction,previousLoad:reference,currentLoad:candidate,targetLoad:candidate,repTarget:target,
      startWorkingIndex:startWorkingIndex,
      action:'Keep this load or reduce further',
      message:'Technique or pain feedback overrides progression.',
      prompt:'Do not add load. Reduce further or stop this exercise, and tell your coach.',
      reason:'A technique or pain warning was recorded after the load change, so progression is paused without undoing the athlete’s entered load.',
  ...
  if(hard&&hard.reps!=null&&hard.reps<pres.repFloor){
    return {
      ...
      direction:direction,previousLoad:reference,currentLoad:candidate,targetLoad:direction==='harder'?reference:candidate,repTarget:target,
      startWorkingIndex:startWorkingIndex,
      action:direction==='harder'?'Reduce the load':'Reduce the load again',
      message:'The adjusted load fell below '+pres.repFloor+' reps at a hard effort.',
      prompt:'Reduce the load and regain '+pres.repFloor+' clean reps. '+effort,
      reason:'The changed load missed the bottom of the coach-programmed range at excessive effort, so another conservative reduction is appropriate.'
    };
  }
  if(hard){
    return {
      ...
      direction:direction,previousLoad:reference,currentLoad:candidate,targetLoad:candidate,repTarget:target,
      startWorkingIndex:startWorkingIndex,
      action:direction==='harder'?'Do not increase again':'Hold this reduced load',
      message:'The changed load is harder than prescribed.',
  ...
  if(direction==='easier'){
    return {
      decision:'live_load_decrease',status:'Load Reduced',tone:'blue',loadChanged:true,
      direction:direction,previousLoad:reference,currentLoad:candidate,targetLoad:candidate,repTarget:target,
      startWorkingIndex:startWorkingIndex,
      action:'Aim for at least '+target+' clean reps',
      message:'Load reduced',
      prompt:'Aim for at least '+target+' clean reps. '+effort,
      reason:'The entered working load is lower than the previous setting. Keep the manual change and rebuild from the bottom of the coach-programmed rep range.'
    };
  }
  return {
