...
function refreshStrengthFeedback(i,splitKey){
  ...
  exercises.forEach(function(ex,ei){
    ...
    // live-load decision may replace it when the athlete deliberately changes a
    // working load; partial rep totals never do, and warm-ups stay out.
...
// right now. A weight-only load-change row is enough to reset the current and
...
function _nsLiveProgress(ex,currentEffort,rec,resolvedName,history,previousEffort){
  ...
  return {
    ...
    ahead:!!(liveLoad&&liveLoad.direction==='harder')||(beat!=null&&total>beat)||topped>=wantSets||(assisted&&previousLoad!=null&&currentLoad!=null&&currentLoad<previousLoad),
...
function _nsApplyLiveLoadRecommendation(rec,live){
  ...
  out.arrow=change.tone==='red'?'↻':(change.direction==='easier'?(out.assisted?'↗':'↘'):(out.assisted?'↘':'↗'));
