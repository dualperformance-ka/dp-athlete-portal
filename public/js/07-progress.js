...
function renderAngleGrid(week){
  ...
  ANGLES.forEach(function(angle){
    ...
    if(url){html+='<img src="'+url+'" alt="'+esc(angle)+' progress photo" onerror="angleImgDead(this)" /><div class="aslot-overlay">'+angle+'</div><button aria-label="Remove '+esc(angle)+' progress photo" onclick="deleteAnglePhoto(\''+angle+'\')" style="position:absolute;top:6px;right:6px;z-index:3;background:rgba(0,0,0,.6);border:none;border-radius:var(--radius-pill);width:26px;height:26px;color:#fff;font-size:var(--font-sm);cursor:pointer;display:flex;align-items:center;justify-content:center;line-height:1">×</button>';}
